...
- **🔴 BLOCCANTE (2026-07-06): poddgx02 NON scrive l'output slurm su `/home` NFS → ogni job FAILED ExitCode `0:53`, ZERO log** (né `.out` né `.err`). **PROVATO** con un job MINIMALE (solo `echo`, niente container/vLLM, con 380G di quota libera): fallisce identico su poddgx02. Quindi NON è la nostra pipeline, NON è la quota, NON è vLLM. È il **compute node poddgx02** che non riesce a scrivere `#SBATCH --output`/`--error` sul path `/home/.../runs/<id>/` (che esiste, u0044-owned, scrivibile dal login node). `dgx_p4_submit` pre-crea già `runs/<id>/` (commento nel codice lega esplicitamente `0:53` al path output). **Regressione lato cluster da giugno** (lo Stadio 2 girò con log a giugno; da allora poddgx02 è cambiato). Il default `dgx_config()$nodelist="poddgx02"` è il sospetto: poddgx01/poddgx03 potrebbero essere sani (la memoria `p4_dgx_paths` già suggerisce `nodelist=NULL` se poddgx02 è problematico). **FIX da provare (mattina)**: (a) submittare su poddgx01/03 (poddgx01 era `IDLE+COMPLETING+PLANNED`, ritentare quando idle); (b) chiedere agli admin di controllare il mount `/home` NFS + slurmd su poddgx02; (c) `--output` su path node-local se necessario. Vedi memoria `dgx_storage_projects_not_home`.
