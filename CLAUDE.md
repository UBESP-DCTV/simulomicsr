...
- **⚠️ STORAGE: usare `/mnt/projects/dctv/dgx/u0044/` (NFS di gruppo, multi-PB) per i dati DGX pesanti, NON `/home/u0044/`** (scoperto 2026-07-06). Il `/home` di u0044 ha una **quota per-utente da 500G** (il filesystem fisico è 4.9 PB al 14%: `df -h /home/u0044` mostra la QUOTA "99%/9.9G liberi", NON il disco). `/mnt/projects` (stesso NFS `ps01nfs.cluster.lan`, export `/projects`) è spazio gruppo `dctv_dgx` **senza cap 500G**; `/mnt/projects/dctv/dgx/u0044/` è scrivibile da u0044 (la root `/mnt/projects/dctv` NO). **Sintomo quota piena** = job FAILED, ExitCode `0:53`, **ZERO log slurm** (vLLM non riesce a scrivere la compile-cache torch/inductor su `~/.cache`=/home → ENOSPC/EDQUOT; assomiglia al bug path `/mnt/home` ma è quota). Nota: fu `sc-gpu-benchmark` (170G, progetto separato) a saturare la quota, non simulomicsr (117G di cui HF_HOME modello 101G). **TODO**: migrare il workspace `simulomicsr-dgx` (HF_HOME + runs) su `/projects` e cambiare il default `dgx_config()$remote_root`; PRIMA verificare che `/mnt/projects` sia montato sui compute node (poddgx02) e bindarlo in `inst/dgx/slurm/run_p4.sh` (i job devono leggere il modello da lì). Fix alternativo/complementare: settare `TMPDIR`/`TORCHINDUCTOR_CACHE_DIR`/`XDG_CACHE_HOME`/`VLLM_CACHE_ROOT` su uno scratch fuori quota in `run_p4.sh`.
