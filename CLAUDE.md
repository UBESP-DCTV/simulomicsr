...
- **🔴 BLOCCANTE (2026-07-06/07): su poddgx02 `autofs` è INACTIVE → la home NFS non è montata → ogni job FAILED ExitCode `0:53`, ZERO log.** CAUSA RADICE PROVATA via `srun` sul compute node: `mount | grep nfs` mostra SOLO `master:/cm/shared`; `/home` è una dir LOCALE VUOTA; `/home/u0044` e `/mnt/home/u0044` NON esistono; `/mnt/projects` è dir non montata; `systemctl is-active autofs` = **inactive**; `cd /home/u0044` = "No such file or directory". slurm non trova `/home/u0044` per `--chdir`/`--output` → `0:53` + zero log. **NON è il nostro codice, NON la quota, NON la config** (identica ai job di giugno 20710/24022 che girarono su poddgx02 con `--output=/home/...` e SCRISSERO i log — allora autofs era attivo). Le mie due diagnosi notturne (quota; "poddgx02 non scrive NFS") erano imprecise: la verità è **autofs spento → nessuna home montata**. **FIX (è CLUSTER, non codice): admin UniPD riavviano/riparano autofs su poddgx02** (`systemctl restart autofs` / rimontare `/home`) — poi la pipeline gira senza toccare una riga. Alternativa: un nodo con autofs attivo (poddgx03 se in partizione; poddgx01 escluso dall'utente). Diagnostica utile: `srun --nodelist=poddgx02 ... /usr/bin/bash -c "mount; systemctl is-active autofs"` (I/O forwarding bypassa l'NFS rotto). Vedi memoria `dgx_storage_projects_not_home`.
