...
test_that("l'apertura precede il quadro d'insieme, che precede i case study, che precedono l'appendice", {
  # struttura "a imbuto" (spec 3.1-3.4): l'ordine delle quattro sezioni non e'
  # negoziabile -- l'apertura (la prova agonista/antagonista) deve arrivare
  # PRIMA di qualunque figura tecnica (criterio 7 della spec).
  qmd <- paste(readLines(system.file("templates", "layer-b-report.qmd",
                                     package = "simulomicsr")), collapse = "\n")
  pos_apertura <- regexpr("(?i)##\\s*apertura", qmd, perl = TRUE)
  pos_corpus   <- regexpr("(?i)##\\s*il corpus", qmd, perl = TRUE)
  pos_case     <- regexpr("(?i)##\\s*i case stud", qmd, perl = TRUE)
  pos_append   <- regexpr("(?i)##\\s*appendice", qmd, perl = TRUE)

  expect_true(all(c(pos_apertura, pos_corpus, pos_case, pos_append) > 0))
  expect_lt(pos_apertura, pos_corpus)
  expect_lt(pos_corpus, pos_case)
  expect_lt(pos_case, pos_append)
})

test_that("l'apertura cita il controllo biologico DHT/enzalutamide coi numeri verificati", {
...
test_that("ogni case study usa la sola etichetta leggibile nel titolo generato, mai il cluster_id", {
  ...
  qmd <- readLines(system.file("templates", "layer-b-report.qmd",
                               package = "simulomicsr"))
  riga_titolo <- grep('###.*label_paper', qmd, value = TRUE, fixed = FALSE)
...
test_that("render_layer_b_report produce un HTML con l'apertura prima delle immagini dei case study", {
  ...
  pos_apertura <- regexpr("(?i)apertura", html, perl = TRUE)
  pos_prima_immagine <- regexpr("<img", html, fixed = TRUE)
  expect_true(pos_apertura > 0)
  expect_true(pos_prima_immagine > 0)
  expect_lt(pos_apertura, pos_prima_immagine)
