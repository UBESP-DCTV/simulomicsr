#!/usr/bin/env Rscript
# analysis/p4-fase-f7-stage3-v9-pre.R --- RED ALERT FASE F6 v9 / Task 8:
# RE-CLUSTER Stadio 3 v9-pre col rework "recupero-nome" (name-recovery).
#
# COSA FA
...
# OUTPUT: analysis/p4-output/<UTC>-stage3-v9pre-<run_id>/ (gitignored, schema v3 +
...
cli::cli_h1(sprintf("Re-cluster Stadio 3 v9-pre (name-recovery deterministico + fix STR:/CHEMBL:, NO overlay LLM) -- modalita': %s",
...
suffix  <- if (SMOKE) sprintf("v9presmoke-%s", run_id) else sprintf("v9pre-%s", run_id)
...
cli::cli_alert_info("run_id={run_id}  (NB: deterministico da input+config; il token v9pre nella dir lo distingue da v3/v4/v5/v6/v7/v8)")
