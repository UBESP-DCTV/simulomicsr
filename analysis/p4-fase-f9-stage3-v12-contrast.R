#!/usr/bin/env Rscript
# analysis/p4-fase-f9-stage3-v12-contrast.R --- RE-CLUSTER Stadio 3 v12:
# MATERIALIZZA L'ANCORAGGIO DAL-CONTRASTO (ADR-0025).
#
...
#
# PAVIMENTI BANDIERA attesi sul risultato vero (censimento 2026-07-27, dopo le
# tre correzioni del 2026-07-26 -- NB: smoke-bandiera.csv e' ANTERIORE e stale):
#   SARS-CoV-2 NCBITaxon:2697049 k>=28 · TGFB1 HGNC:11766 k>=38 ·
#   LPS CHEBI:16412 k>=26 · enzalutamide CHEBI:68534 k>=24 ·
...
#       Rscript analysis/p4-fase-f9-stage3-v12-contrast.R
#   FULL RUN (gate utente separato, ~8h; NON lanciare senza ok; usare setsid):
#       SMOKE=0 Rscript analysis/p4-fase-f9-stage3-v12-contrast.R
...
cli::cli_h1(sprintf("Re-cluster Stadio 3 v12 (ancoraggio dal-contrasto ADR-0025 + name-recovery + 2 overlay LLM) -- modalita': %s",
...
suffix  <- if (SMOKE) sprintf("v12smoke-%s", run_id) else sprintf("v12-%s", run_id)
...
cli::cli_h1("SANITY v12")
cli::cli_alert_info("run_id={run_id}  (NB: deterministico da input+config; il token v12 nella dir lo distingue da v3..v10)")
...
if (dir.exists(v3_dir)) {
  ...
  scope <- if (SMOKE) "(v3 GLOBALE vs v12 SUBSET: non confrontabili 1:1, solo ordine di grandezza)" else "(v3 GLOBALE vs v12 GLOBALE)"
...
cli::cli_h2("SANITY v12 (overlay + bandiera)")
...
cli::cli_h2("SANITY v12 (ancoraggio dal-contrasto)")
