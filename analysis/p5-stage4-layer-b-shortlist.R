# analysis/p5-stage4-layer-b-shortlist.R
# Genera una shortlist di 30-50 cluster_id candidati per il Layer B paper-grade
# a partire dal Layer A output (cluster_pooled.parquet, 13.7M righe) + Stage 3
# metadata (anchor v3, kind_effective/agent_id/tissue).
#
# Output: analysis/p4-output/layer-b-shortlist.csv (committable, paper-grade).
#
...
#
# Smoke pin: i 3 cluster gia' validati nel smoke 3-cluster (CLAUDE.md
# 2026-05-24, tag p5-stadio4-layer-b-complete) sono inclusi come "pin" nella
# shortlist a prescindere dai filtri:
#   - group_L0_a6f8c0e9 (mega_big)
#   - pair_L2_3ce85e50  (mega_aug)
#   - group_L0_1a0673ae (mega_small)
#
# Usage:
#   Rscript analysis/p5-stage4-layer-b-shortlist.R
#
# Pre-requisiti:
#   - analysis/p4-output/layer-b-cluster-summary.rds (generato da workflow
#     esplorativo upstream; questo script lo rigenera se manca)
#   - Layer A output in analysis/p4-output/20260523T032601Z-stage4-96c43acb/
#   - Stage 3 output in analysis/p4-output/20260519T055547Z-stage3-2153addc/
...
summary_rds <- "analysis/p4-output/layer-b-cluster-summary.rds"
out_csv    <- "analysis/p4-output/layer-b-shortlist.csv"

# Smoke validati (CLAUDE.md tag p5-stadio4-layer-b-complete)
SMOKE_PINS <- c(
  "group_L0_a6f8c0e9",   # mega_big
  "pair_L2_3ce85e50",    # mega_aug
  "group_L0_1a0673ae"    # mega_small
...
build_summary <- function() {
  ...
  final <- agg |>
    ...
    select(cluster_id, method, mode, level,
           kind_effective, agent_id, tissue,
...
out_tbl <- shortlist |>
  ...
  select(
    ...
    label_paper_suggested, kind_effective, agent_id, tissue,
    k_effective, n_baseline_augmented, n_total, n_treated, n_control,
