# analysis/p4-fase-f5-stage4-layer-a-rebuild-v8.R --- RED ALERT FASE F6.
# Layer A full run Stadio 4 v8 sul MEDESIMO Stadio 3 v7 + master v3, con il
# NUOVO ramo di pooling `rem_group` (meta-analisi cross-studio nominate L2-L4).
#
# Differenze vs v7 (rebuild-v7.R):
#   - CODICE: build_stage4_results ora ammette i cluster group nominati (porta
#     rem_group in .identify_layer_a_clusters, senza gate safety_min), costruisce
#     il dispatch group-aware (.build_group_rem_dispatch_from_stage3, control
#     in-study via comparisons), instrada method="rem_group" al REM per-studio
#     con cutoff k_eff (studi distinti) e COLLAPSE dei bracci intra-studio
#     (.collapse_arms_by_study, inverse-variance FE) prima del REM.
#   - INPUT INVARIATI: stage3_dir=v7 (NON serve re-cluster), master v3, H5.
#   - out_dir -> simulomicsr-stage4-v8, token -stage4-v8-.
#   - schema_versions.rem_group_strategy = v1_per_study_rem_named_groups (cache-bust).
#
# Config invariata vs v7 (uniformity): MEGA-AUG bidirezionale,
...
#
# Usage (full, detached): setsid Rscript analysis/p4-fase-f5-stage4-layer-a-rebuild-v8.R \
...
cli_h1(paste0("Stadio 4 Layer A rebuild v8 (ramo rem_group)", if (DRY_RUN) " [DRY_RUN]" else ""))
...
out_dir <- file.path("/mnt/wwn-0x5000039d58caca35/simulomicsr-stage4-v8",
  sprintf("%s-stage4-v8-%s", format(Sys.time(), "%Y%m%dT%H%M%SZ", tz = "UTC"),
...
cli_alert_success("Layer A rebuild v8 OK")
