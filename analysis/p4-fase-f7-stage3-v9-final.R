#!/usr/bin/env Rscript
# analysis/p4-fase-f7-stage3-v9-final.R --- RED ALERT FASE F6 v9 / Task 9:
# RE-CLUSTER Stadio 3 v9-final col rework "recupero-nome" (name-recovery)
# PIU' l'overlay delle correzioni LLM name-cleanup (side-table Mistral, Task 12).
#
# COSA FA
...
#
# COME LANCIARLO
#   SMOKE (default; gira in pochi minuti su un subset di studi che TARGETIZZA i
#   minestroni v3 -- questo lo esegue chi valida lo script):
#       Rscript analysis/p4-fase-f6-stage3-reclustering.R
#   FULL RUN (gate utente separato, ~ore; NON lanciare senza ok):
#       SMOKE=0 Rscript analysis/p4-fase-f6-stage3-reclustering.R
#   Parametri opzionali via env:
...
cli::cli_h1(sprintf("Re-cluster Stadio 3 v9-final (name-recovery deterministico + overlay correzioni LLM name-cleanup) -- modalita': %s",
...
# ---------------------------------------------------------------------------
# 6b. Overlay correzioni LLM (name-cleanup v9): GSM -> identita' corretta.
#     Precision-gated: SOLO le righe action=="override" della side-table Mistral
#     (Task 4/5). Agganciato ai GSM membri dei cluster v9-pre rivisti -> nessun
#     rischio di over-correction globale sul nome. La side-table e' OPZIONALE:
#     senza SIDE_TABLE lo script produce il v9-pre-equivalente (retrocompat).
#     STAGE3_V9PRE_DIR = dir v9-pre (mappa cluster_id -> record_id della side-table).
# ---------------------------------------------------------------------------
side_path <- Sys.getenv("SIDE_TABLE", "")
if (nzchar(side_path)) {
  cli::cli_h2("6b. Overlay correzioni LLM (name-cleanup)")
  v9pre_dir <- Sys.getenv("STAGE3_V9PRE_DIR", "")
  if (!nzchar(v9pre_dir))
    stop("SIDE_TABLE settato ma STAGE3_V9PRE_DIR mancante: serve la dir v9-pre ",
         "per mappare i cluster_id della side-table -> record_id -> GSM.")
  stopifnot(file.exists(side_path), dir.exists(v9pre_dir))
  side      <- readRDS(side_path)
  v9pre_asg <- load_stage3(v9pre_dir)$assignments          # cluster_id -> record_id (v9-pre)
  source("analysis/audit/_gsm-lookup-helper.R")
  rec_env   <- build_record_gsm_lookup(stage2_path)        # record_id -> GSM sample_ids
  r2g <- function(rid) {
    g <- get0(rid, envir = rec_env, inherits = FALSE)
    if (is.null(g)) character(0) else as.character(g)
  }
  overlay         <- simulomicsr:::.side_table_to_recovery_overlay(side, v9pre_asg, r2g)
  recovery_lookup <- simulomicsr:::.overlay_recovery_lookup(recovery_lookup, overlay)
  n_ov <- sum(side$action == "override", na.rm = TRUE)
  cli::cli_alert_success(sprintf(
    "Overlay LLM applicato: %d GSM corretti (da %d righe override della side-table)",
    length(overlay), n_ov))
} else {
  cli::cli_alert_info("SIDE_TABLE non settato: nessun overlay LLM (output = v9-pre-equivalente).")
}
...
suffix  <- if (SMOKE) sprintf("v9smoke-%s", run_id) else sprintf("v9-%s", run_id)
...
cli::cli_h1("SANITY v9-final")
cli::cli_alert_info("run_id={run_id}  (NB: deterministico da input+config; il token v9 nella dir lo distingue da v3/v4/v5/v6/v7/v8/v9pre)")
...
if (dir.exists(v3_dir)) {
  ...
  scope <- if (SMOKE) "(v3 GLOBALE vs v6 SUBSET: non confrontabili 1:1, solo ordine di grandezza)" else "(v3 GLOBALE vs v6 GLOBALE)"
  cli::cli_inform("CONFRONTO disease_vs_normal|UNK cluster: v3={v3_dis_unk} -> v6={sum(dis & is_unk)} {scope}")
}
