# analysis/p4-fase-f5-stage4-layer-a-rebuild-v10.R --- RED ALERT FASE F6 v10.
# Layer A full run Stadio 4 v10 sullo Stadio 3 v10 (recupero-nome perturbativo B
# + name-cleanup v9 + LLM-fallback finale STR:/UNK) + master v3, MEDESIMO ramo
# di pooling `rem_group` (meta-analisi cross-studio nominate L2-L4) di v8/v9.
# Identico al rebuild v9: cambia SOLO out_dir/token (v9 -> v10). STAGE3_DIR via env.
#
# Differenze vs v8 (rebuild-v8.R):
#   - INPUT: stage3_dir NON hardcoded — viene dalla env var STAGE3_DIR (la dir
#     Stadio 3 v9-final e' prodotta da un run gated a valle, non nota a code
#     time). h5_path/stage2_path invariati (master v3, H5).
#   - CODICE: NESSUNA modifica — stesso build_stage4_results, stesso ramo
#     rem_group, stessa config (config uniformity — nessun mixing sampling/
#     infra tra stadi).
#   - out_dir -> simulomicsr-stage4-v10, token -stage4-v10-.
#
# Config invariata vs v8/v9 (uniformity): MEGA-AUG bidirezionale,
# max_baseline_per_arm=350, dream_workers_cap=32, de_engine mega/mega_aug=dream.
#
# DRY_RUN=1 -> si ferma dopo identificazione Layer A + conteggio sample (no DE).
#
# Usage (full, detached): STAGE3_DIR=<dir Stadio 3 v10> setsid \
#   Rscript analysis/p4-fase-f5-stage4-layer-a-rebuild-v10.R \
#   > analysis/p4-fase-f5-stage4-layer-a-rebuild-v10.log 2>&1 < /dev/null &
...
cli_h1(paste0("Stadio 4 Layer A rebuild v10 (Stadio 3 v10, ramo rem_group)", if (DRY_RUN) " [DRY_RUN]" else ""))

stage3_dir <- Sys.getenv("STAGE3_DIR", "")
if (!nzchar(stage3_dir)) stop("STAGE3_DIR non settato: passare la dir Stadio 3 v9-final (SMOKE=0) o una dir esistente per il DRY_RUN.")
...
config$mega_aug$legacy_monodirectional <- FALSE  # bidirezionale (= v7)
...
out_dir <- file.path("/mnt/wwn-0x5000039d58caca35/simulomicsr-stage4-v10",
  sprintf("%s-stage4-v10-%s", format(Sys.time(), "%Y%m%dT%H%M%SZ", tz = "UTC"),
...
cli_alert_success("Layer A rebuild v10 OK")
