...
r <- readRDS(file.path(SC, "fase1-v8-results.rds")); pm <- r$pm; agg <- r$agg
elig <- pm[pm$elig, ]
agg <- agg |> arrange(desc(k))
con <- file(file.path(SC, "v8-census-bundles.txt"), "w")
writeLines(sprintf("# CENSIMENTO COERENZA v8 — %d cluster poolabili k>=3 (TUTTI)", nrow(agg)), con)
...
close(con)
cat("scritto v8-census-bundles.txt per", nrow(agg), "cluster\n")
cat("righe:", length(readLines(file.path(SC, "v8-census-bundles.txt"))), "\n")
