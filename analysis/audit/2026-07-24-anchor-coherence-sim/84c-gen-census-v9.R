...
con <- file(file.path(SC, "v9-census-bundles.txt"), "w")
writeLines(sprintf("# CENSIMENTO COERENZA v9 — %d cluster poolabili k>=3 (TUTTI)", nrow(agg)), con)
...
close(con)
cat("scritto v9-census-bundles.txt per", nrow(agg), "cluster\n")
cat("righe:", length(readLines(file.path(SC, "v9-census-bundles.txt"))), "\n")
