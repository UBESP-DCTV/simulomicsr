...
r <- readRDS(file.path(SC, "fase1-v10-results.rds")); pm <- r$pm; agg <- r$agg
elig <- pm[pm$elig, ]
agg <- agg |> arrange(desc(k))
con <- file(file.path(SC, "v11-census-bundles.txt"), "w")
writeLines(sprintf("# CENSIMENTO COERENZA v11 (post-scarto righe) — %d cluster poolabili k>=3 (TUTTI)", nrow(agg)), con)
...
close(con)
cat("scritto v11-census-bundles.txt per", nrow(agg), "cluster\n")
cat("righe:", length(readLines(file.path(SC, "v11-census-bundles.txt"))), "\n")
