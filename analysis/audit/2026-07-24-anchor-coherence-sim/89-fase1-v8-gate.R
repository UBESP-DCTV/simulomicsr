...
combo_from_labels <- function(tl, cl) {
  ...
  # v8: anche la virgola separa agenti ("virus positive at AV, RSV, rhinovirus")
...
cat(sprintf("\n=== v8 poolabili k>=3: %d (v6 196) | k>=5: %d (v6 70) ===\n", nrow(agg), sum(agg$k >= 5)))
...
saveRDS(list(pm = pm, agg = agg), file.path(SC, "fase1-v8-results.rds"))
