# GATE v11 — v10 con le regole prese dal CODICE DI PACCHETTO.
#
# Le regole non vivono piu' in questo script: sono in R/stage3-contrast-gate.R e
# R/stage3-row-pairing.R, con i loro test. Qui restano solo il flusso e le parti
# non ancora portate (entita', combo, dedup). Il confronto 108-equivalenza-gate.R
# ha mostrato che le versioni di pacchetto correggono quattro punti in cui lo
# script non vedeva oltre un underscore ("_doxycycline" non era un induttore,
# "Baseline_Control" non era una baseline, "CON_1_input" non era un controllo di
# saggio, "Patient_081" non era un contesto clinico) e aggiungono "pulmonary" al
# vocabolario anatomico. Questo gate rimisura tutto con quelle regole.
#
...
## --- le regole vengono dal PACCHETTO (R/stage3-contrast-gate.R) ---------------
is_generic_tok      <- .cg_is_generic_token
is_inducer_name     <- .cg_is_inducer
verso_of            <- .cg_direction
material_arm        <- function(lab, fl) .cg_material_arm(lab, fl)
baseline_kind       <- .cg_baseline_kind
infection_ctx       <- function(cls, cl, tl = "") .cg_infection_context(cls, cl, tl)
resistance_mismatch <- .cg_resistance_mismatch
is_multiclass       <- .cg_is_multiclass
broken_contrast     <- .cg_broken_contrast
anatomy_of          <- .cg_anatomy
celltype_of         <- .cg_celltypes
distinctive_toks    <- .cg_distinctive_tokens
match_all_words     <- .cg_matches_all_words
is_noncontrol       <- .cg_is_noncontrol
is_ctrl_like        <- .cg_is_control_like_treated
is_umbrella         <- .cg_is_umbrella_name
difetto_riga <- function(tl, cl, cls, ent, extra_stop) {
  k <- paste(tl, cl, cls, ent, sep = "\r")
  if (exists(k, envir = .rr_cache_row, inherits = FALSE)) return(get(k, envir = .rr_cache_row))
  v <- .rp_row_defect(tl, cl, cls, ent, extra_stop, oe, .rr_cache_tok)
  assign(k, v, envir = .rr_cache_row); v
}
...
for (i in seq_len(n)) {
  ...
  if (is_noncontrol(pm$control_label[i])) {
  ...
  if (startsWith(e, "NAME:") && is_umbrella(nm_low)) { dr[i] <- "nome_ombrello"; next }
  ...
  if (nzchar(rn) && is_umbrella(rn)) { dr[i] <- "nome_ombrello"; next }
...
cat(sprintf("\n=== v11 poolabili k>=3: %d (v10 144) | k>=5: %d ===\n", nrow(agg), sum(agg$k >= 5)))
...
saveRDS(list(pm = pm, agg = agg), file.path(SC, "fase1-v11-results.rds"))
cat("\nsalvato fase1-v11-results.rds\n")
