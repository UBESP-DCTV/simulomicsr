# Che cosa cambia fra il gate v10 (script, 144 poolabili) e il gate v11 (stesse
# regole prese dal CODICE DI PACCHETTO). Serve a sapere quali gruppi vanno
...
v9 <- readRDS(file.path(SC, "fase1-v10-results.rds"))   # riferimento: gate v10
v10 <- readRDS(file.path(SC, "fase1-v11-results.rds"))  # nuovo: gate v11 (codice di pacchetto)
ver <- read.csv(file.path(SC, "v11-census-verdicts-FINAL.csv"), stringsAsFactors = FALSE)
...
saveRDS(list(persi = persi, nuovi = nuovi, cambiati = camb, invariati = inv),
        file.path(SC, "delta-v10-v11.rds"))
