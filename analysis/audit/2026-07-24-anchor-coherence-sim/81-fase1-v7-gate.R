# GATE v7 — anchor derivato-dal-contrasto, residuo v6 chiuso.
#
...
norm_part <- function(p) {
  ...
  w <- w[nzchar(w) & nchar(w) >= 3 & !(w %in% GENERIC) & !(w %in% CONNECTORS)]
  if (length(w)) paste(w, collapse = " ") else ""
}
...
combo_from_labels <- function(tl, cl) {
  s <- drop_doses(tl); c0 <- drop_doses(cl)
  parts <- trimws(strsplit(s, "\\s*[+&]\\s*|\\s+and\\s+|\\s+plus\\s+|\\s*/\\s*", perl = TRUE)[[1]])
  if (length(parts) < 2) return(character(0))
  ctrl_ag <- unique(vapply(trimws(strsplit(c0, "\\s*[+&]\\s*|\\s+and\\s+|\\s*/\\s*", perl = TRUE)[[1]]),
                           function(p) agent_id_of(norm_part(p)), ""))
  ag <- character(0); nm <- character(0)
  for (p in parts) {
    np <- norm_part(p); if (!nzchar(np)) next
    a <- agent_id_of(np)
    if (nzchar(a) && !(a %in% ctrl_ag)) { ag <- c(ag, a); nm <- c(nm, np) }
  }
  if (length(unique(ag)) >= 2) unique(nm) else character(0)
}
...
for (i in seq_len(n)) {
  ...
  ct[i] <- paste0(ctb, mat[i], bas[i],
                  infection_ctx(if (is_path) "infection" else cls, pm$control_label[i]),
                  class_sig(pm$dclasses[i]))
  # R3 — verso
  ...
  cp <- combo_parts(tval)
  if (!length(cp)) cp <- combo_from_labels(pm$treated_label[i], pm$control_label[i])
...
cat(sprintf("\n=== v7 poolabili k>=3: %d (v6 196) | k>=5: %d (v6 70) ===\n", nrow(agg), sum(agg$k >= 5)))
...
saveRDS(list(pm = pm, agg = agg), file.path(SC, "fase1-v7-results.rds"))
cat("\nsalvato fase1-v7-results.rds\n")
